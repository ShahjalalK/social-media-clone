...
const Login = (props: Props) => {
  const {login, signInUser, signInLoading, signInerror} = FirebaseAuthApi()
  ...
  return (
    <form onSubmit={submitHandler} className="drop-shadow rounded-lg p-5 bg-white max-w-sm mx-auto flex flex-col space-y-5">
      ...
      <div className="flex flex-col space-y-1">
        ...
        {signInerror && <p className="text-sm text-red-600">{FIREBASE_ERROR[signInerror.message as keyof typeof FIREBASE_ERROR]}</p>}
       ...
       <span className="font-medium text-blue-700 hover:bg-blue-200 hover:underline cursor-pointer rounded-full p-1"  onClick={() => setModalState((prev) => ({...prev, view: "resetPassword"}))}>Forgot password?</span>
        ...
        <button type='submit' className="w-full rounded-full py-2 text-lg bg-[#0072b1] text-white hover:bg-blue-800 transition-colors duration-100 focus:ring-4 focus:ring-blue-300 font-medium px-5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">{signInLoading ? <Spinner /> : "Sign in"}</button>
